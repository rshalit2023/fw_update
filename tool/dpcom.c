...
void dp_get_page_data(DPULONG image_requested_address)
{
    ...
    */
    if (fp) {
        rc = fseek(fp, image_requested_address, SEEK_SET);
        if (0 != rc)
            {
                printf("%s:%d fseek err[%d]\n", __func__, __LINE__, rc);
            }
        else{
        }
    ...
    numReadBytes = dp_read_dat_file(page_global_buffer,return_bytes);
    start_page_address = image_requested_address;
    end_page_address = image_requested_address + return_bytes - 1;
    if(numReadBytes != return_bytes){
        printf("%s asked reading %d actual read %d\n", return_bytes, numReadBytes);
        while(1);
    }
#endif	
    return;
