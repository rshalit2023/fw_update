...
DPUINT global_uint2;
    ...
    #if defined(ENABLE_DISPLAY) || defined(ENABLE_DISPLAY_PROGRESS)
    ...
    void dp_display_value(DPULONG value,DPUINT descriptive)
    {
        switch(descriptive){
            ...
            default:{
                printf("%s:%d unkown format %d\n", __func__, __LINE__, descriptive);
            }
        }
        return;
    }
...
int dp_read_dat_file(char* buf, unsigned int readsz)
{
  size_t bytes = 0;

  if(fp)
  {
    bytes = fread (buf, 1, readsz, fp);
    if(bytes!=readsz)
    {
        printf("Try to read %u bytes but actual read is %lu\n", readsz, bytes);
    }

  }
  else
  {
    printf ("error: fp is NULL %p\n", fp);
  }
  return bytes;
}
