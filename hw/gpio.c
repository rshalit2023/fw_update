...
int setGpioOutGpioIn(char *gpioOut, char *gpioIn){
    ...
    //if(argc<3) {
        printf("Usage: gpio_usage_sysfs <gpio-out:%s> <gpio-in:%s>\r\n", gpioOut, gpioIn);
    //    exit(-1);
    //}

    // We reached here so params are OK - not checking for a legal number!
    gpio_out = atoi(gpioOut); //argv[1]);
    gpio_in = atoi(gpioIn); //argv[2]);
...
//    printf("%s gpioId:%s gpio_dir:%s val:%d\r\n", __func__, gpioId, (gpio_dir==GPIO_IN)?DIR_IN: DIR_OUT, val);
    ...
    gpio_unexport(gpio);
    gpio_export(gpio);
    gpio_set_direction(gpio,(gpio_dir==GPIO_OUT)?DIR_OUT: DIR_IN);

	if(gpio_dir==GPIO_OUT)
		gpio_set_value(gpio,(val>0)?VALUE_HIGH: VALUE_LOW);
	else
		gpio_set_edge(gpio,(val>0)?EDGE_RISING: EDGE_FALLING);
